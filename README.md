...
`/reports`에서는 생성된 JSON 리포트 이력을 목록으로 확인하고, 각 리포트의 대시보드 보기와 다운로드 링크를 사용할 수 있습니다.
...
Flask 서버 실행 후 `/rules`로 접속하면 Rule ID, 공격 유형, 위험도, 신뢰도, 탐지 패턴, 권장 대응을 웹 화면에서 조회할 수 있습니다.
